...
export const HomePage: React.FC<HomePageProps> = ({ setActiveTab, openLeadModal }) => {
  ...
  return (
    <div className="min-h-screen bg-[#0B1B2B] text-white selection:bg-[#C5A059] selection:text-[#0B1B2B]">
      
      {/* HERO SECTION WITH GEOMETRIC OVERLAY */}
      <section className="relative pt-12 pb-24 lg:pt-20 lg:pb-32 overflow-hidden bg-geo-pattern">
        {/* Subtle Gradient Glow */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full max-w-7xl h-96 bg-gradient-to-b from-[#142A3E]/60 via-transparent to-transparent pointer-events-none" />

        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          
          {/* Top Institutional Badge & Circular Logo Emblem */}
          <div className="flex flex-col items-center justify-center mb-6">
            <Logo variant="circle" height={64} className="mb-4" />
            <div className="inline-flex items-center space-x-2 bg-[#142A3E]/80 border border-[#C5A059]/40 px-4 py-1.5 rounded-full text-xs font-semibold text-[#C5A059] uppercase tracking-widest shadow-md">
      ...
      {/* INTERACTIVE HOVER CARDS: INBOUND vs OUTBOUND (PRD Page 1 Spec) */}
      ...
      {/* 5-CARD GRID DISPLAYING CORE, SUPPORTING, AND DELIVERY SERVICE LINES (PRD Page 1 Spec) */}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {SERVICE_LINES.map((service, idx) => (
              <div
                key={service.id}
                className={`p-6 rounded-xl bg-white border border-[#E2E8F0] shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col justify-between ${
                  idx === 0 ? 'lg:col-span-1' : ''
                }`}
              >
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] font-semibold uppercase tracking-widest px-2.5 py-1 rounded bg-[#0B1B2B] text-[#C5A059]">
                      {service.category}
                    </span>
                  </div>
                ...
                <div className="pt-6">
                  <button
                    onClick={() => setActiveTab('services')}
                    className="w-full text-left text-xs font-semibold uppercase tracking-wider text-[#0B1B2B] hover:text-[#C5A059] flex items-center justify-between pt-2 border-t border-gray-100"
                  >
      ...
      {/* 4-COLUMN RESPONSIVE GRID DISPLAYING PACKAGED OFFERS (PRD Page 1 Spec) */}
      <section className="py-20 bg-[#0B1B2B] border-t border-[#142A3E]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
